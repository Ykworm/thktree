...
## 简介
本文件面向初学者与高级开发者，系统性地文档化聊天界面组件，涵盖 ChatScreen 架构与用户交互流程、ChatListView 的消息渲染与滚动行为、ChatComposer 的输入与发送控制、MessageBubble 的样式与交互，以及新增的模型选择面板和LLM提供商管理系统。文档还详细介绍了上下文使用量指示器的设计、实时模型可用性信息集成，以及对话级模型配置管理。同时提供扩展与定制建议，帮助你在不破坏现有架构的前提下，安全地添加新消息类型、交互功能与视觉效果。
...
- LLM提供商配置：lib/data/models/llm_provider_config.dart
- LLM模型配置：lib/data/models/llm_model_config.dart
...
- [lib/ui/core/shared/chat_composer.dart:1-181](file://lib/ui/core/shared/chat_composer.dart#L1-L181)
...
- [lib/ui/features/llm/llm_providers_screen.dart:1-122](file://lib/ui/features/llm/llm_providers_screen.dart#L1-L122)
...
- **ChatComposer**：多行文本输入、回车发送、Shift+回车换行、发送/停止按钮联动、输入校验与错误恢复，支持模型选择按钮。
...
- **LLM提供商管理系统**：包括提供商配置、模型配置、配置存储、模型获取器和提供商管理界面。
...
**章节来源**
- [lib/ui/core/shared/chat_composer.dart:5-181](file://lib/ui/core/shared/chat_composer.dart#L5-L181)
...
### LlmConfigStore：配置存储
- **存储机制**：独立的JSON文件存储提供商配置，API Key通过flutter_secure_storage单独加密存储。
...
- **多提供商支持**：支持OpenAI兼容接口、Anthropic、Gemini等不同提供商的API。
...
- **ChatComposer** 依赖外部回调 onSend 与 onStopStreaming，实现输入与发送控制的解耦，支持模型选择回调。
...
- **LLM提供商管理系统** 形成完整的配置管理闭环，包括配置存储、模型获取、迁移支持等功能。
...
**重大更新**：本次现代化改造引入了全新的模型选择面板和完整的LLM提供商管理系统，包括：
- **模型选择面板**：提供腾讯风格上推布局，支持实时模型可用性信息显示
- **LLM提供商管理**：支持多提供商配置、模型列表获取、API Key安全存储
- **对话级模型配置**：每个对话可独立配置提供商和模型
- **预置提供商系统**：包含迁移和初始化逻辑，确保配置完整性

对于初学者，建议从 ChatScreen 的集成与 MessageBubble 的基础样式入手；对于高级开发者，可在 ChatController 扩展更多消息类型、在 MessageBubble 增强交互、在 ChatListView 优化滚动策略，或在 LLM提供商管理系统中扩展新的提供商类型。
