...
> 注记（2026-07-21）：上述「无法计算滚动位置」的**论据已过时**——`ChatListView.scrollToMessage` 已用 overlay 离屏测量实现变高 item 定位（聊天内搜索/对话目录/Pin 对照栏 Jump 均在使用）。但本条结论针对的「回跳联动」功能本身未做，隔离 Reply Page 方案不受影响。
...
Chat Page（根）
  └─ 点击左上角 more 按钮
       └─ 弹出 Sheet（现有更多菜单）
            └─ 新增宫格项：本次发言（入口短标签；进入后 List 标题为「我发出的消息」，见第 9 节）
