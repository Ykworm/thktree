...
| Chat-to-Note | notes | ✅ 完成 | 2026-07-21 | [README](modules/notes/README.md) | — | `lib/ui/features/chat/chat_screen.dart` + `lib/ui/core/shared/message_bubble.dart` | assistant 消息存为笔记：action row 右端 **Note** 文字按钮；2026-07-21 二轮改为**延迟创建**——打开 `NoteEditorScreen`（deferCreate + 预填），✓ 才落盘，返回丢弃；另对照栏 To Note 走 `NoteSelectScreen` 追加到既有笔记或新建 |
...
| Context Usage Bar | chat | ⏸ 暂时屏蔽 | 2026-07-23 | [README](modules/chat/README.md) | — | `lib/ui/features/chat/chat_screen.dart`（`_ContextUsageBar`） | 原 1px 无名 token 进度条用户无法理解、低占用时像脏蓝线；**暂时恒 `SizedBox.shrink()`**，调用签名保留；待带文案的可读警示设计后再开 |
...
| Pin 对照栏 | chat | ✅ 完成 | 2026-07-21 | [README](modules/chat/README.md) | — | `lib/data/services/pin_storage.dart` + `lib/data/services/pin_content_loader.dart` + `lib/ui/features/chat/widgets/pin_peek_panel.dart` | 气泡 Pin/Note 文字按钮 + 长按标题行消息菜单，把 message 或整篇笔记钉进 pins.json（上限 5 条满员拦截 + 同锚点去重）；Pin 按钮双态（Pin/Unpin 再点取消，笔记详情页为可拖动悬浮 pin）；右缘把手挂 shell 层（Themes/Notes tab 常驻、可上下拖动），root navigator 居上大卡片面板（深色 scrim + 底部关闭按钮 + 圆点切换 + markdown 预览），卡片动作：Source（跳回原 chat 对应 message / 打开笔记）/ To Note（`NoteSelectScreen`）/ Remove |
...
| Feature | 模块 | 状态 | 最后更新 | README | Visual | 代码路径 | 说明 |
|---------|------|------|----------|--------|--------|----------|------|
| Tree 转 Wiki 快照 | wiki | ✅ 完成 | 2026-07-19 | [README](modules/wiki/README.md) | — | `lib/data/services/wiki_store.dart` + `lib/data/services/wiki_service.dart` + `lib/ui/features/wiki/` | ThemeDetailScreen 新增 Tree/Wiki 双 tab；以单个 tree（root node）为单位生成 `wiki/{rootNodeId}/` 快照；Wiki tab 首页为书籍目录页，点击进入章节阅读页（上一章/下一章导航） |
