...
> 为什么：对话天然是发散的，但发散需要结构才能沉淀。树让「一次聊到哪、为什么这么聊」可被回溯，而不是淹没在时间流里。
...
- **选区工具 / 目录 / 内搜索 / 祖先链面包屑**：在长对话里定位、跳转、复用。

> 为什么：LLM 的价值不只是「回答」，更是「和你一起把想法长出来」。分支与对照让同一话题能多次生长，而不是覆盖掉之前的路径。
...
> 为什么：对话是过程，笔记是结果。把「聊出来的东西」留住，知识树才真正长成。
...
> 为什么：树解决了「组织」，但旧内容会沉底。搜索让任何历史节点都能被重新找到。
...
> 为什么：结构化之外，也需要「意外」。Lab 是鼓励试错、反向激发思考的角落。

### 基础设施

- **LLM Provider 管理**：多 Provider + API Key 配置，统一错误处理与重试。
- **备份与恢复**：本地自动备份（24h 补偿，保留 7 份）、手动备份分享、导入覆盖/合并。
- **语音播放（iOS）**：原生 AVSpeechSynthesizer 朗读对话，带氛围动效。
- **分享 / 生物认证（Face ID）**：把问答渲染成图片分享，进前台生物验证。
- **国际化**：中英双语。
...
- 品牌基础与声音指南：[`docs/BRAND.md`](docs/BRAND.md)
