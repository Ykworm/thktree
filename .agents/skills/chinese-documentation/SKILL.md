...
**与 AGENTS.md：** 「禁止章节符号（U+00A7）」是仓库**常驻红线**（聊天 / docs / `_tmp` / commit 文案一律适用）。本 skill 只展开排版细则；**agent 不得因未触发本 skill 就使用该字符**。
...
| 场景 | 禁止 | 应写 |
|------|------|------|
| 文内交叉引用 | 用章节符拼章节号 | `见第 11.4 节` |
| 清单/待办 | 用章节符指某节 | `确认第 4 节信息架构` |
| 修订记录 | 用章节符指新节 | `增加第 9.0 节：…` |
| 与标题并列 | 用章节符指标题所在节 | `与第 7 节「安全与隐私」一致` |
