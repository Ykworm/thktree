...
description: >
  ...
  禁止章节符号（U+00A7 section sign）的细则在此；**该禁令已写入 AGENTS.md 红线，全会话常驻生效**，
  不依赖本 skill 是否被触发。其余排版细则仅在用户显式 /chinese-documentation 时展开。
---
...
**与 AGENTS.md：** 「禁止章节符号（U+00A7）」是仓库**常驻红线**（聊天 / docs / `_tmp` / commit 文案一律适用）。本 skill 只展开排版细则与对照表；**agent 不得因未触发本 skill 就使用该字符**。
...
### 禁止章节符号 U+00A7（硬规则 · 与 AGENTS 红线同文）

**永远不要使用 Unicode U+00A7（section sign / 章节符号）。**  
适用范围：聊天、`docs/**`、`docs/_tmp/**`、commit/PR、skill 正文。说明本禁令时写 **U+00A7** 或「章节符号」，不要为了举例再把该字符打进正文。

| 场景 | 禁止 | 应写 |
|------|------|------|
| 文内交叉引用 | 用章节符拼「见 11.4」 | `见第 11.4 节` |
| 清单/待办 | 用章节符指第 4 节 | `确认第 4 节信息架构` |
| 修订记录 | 用章节符指第 9.0 节 | `增加第 9.0 节：…` |
| 与标题并列 | 用章节符指第 7 节 | `与第 7 节「安全与隐私」一致` |

原因：在部分字体、终端、IDE、剪贴板链路下显示异常或难输入；中文技术文档用「第 N 节」更清晰。

写完文档 / 回复后自检：全文不得出现 U+00A7。
