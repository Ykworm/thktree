...
List<SessionMessage> _parseMessages(String body) {
  ...
  void flushMessage(int endLineExclusive) {
    ...
    final (status, errorCode, bodyLines) = _extractStatusAndBody(rawBodyLines);
...
(SessionMessageStatus, String?, List<String>) _extractStatusAndBody(
  ...
  if (trimmedTrailingEmpty.isEmpty) {
    return (SessionMessageStatus.done, null, const <String>[]);
  }

  final lastLine = trimmedTrailingEmpty.last;
  if (lastLine == '<!-- streaming -->') {
    trimmedTrailingEmpty.removeLast();
    return (SessionMessageStatus.streaming, null, trimmedTrailingEmpty);
  }

  final errorMatch = RegExp(r'^<!-- error: ([a-z]+) -->$').firstMatch(lastLine);
  if (errorMatch != null) {
    trimmedTrailingEmpty.removeLast();
    return (SessionMessageStatus.error, errorMatch.group(1), trimmedTrailingEmpty);
  }

  return (SessionMessageStatus.done, null, trimmedTrailingEmpty);
}
